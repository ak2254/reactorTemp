    ...
    for work_order, monday_record in monday_lookup.items():
        if work_order not in original_lookup:
            ...
            for key in original_record.keys():
                if key in monday_record:
                    # If the values are different, report the change
                    if monday_record.get(key) != original_record.get(key):
                        fields_changed.append(key)
                        print(f"Column '{key}' has changed:")
                        print(f"  Original value = {original_record[key]}")
                        print(f"  Monday value = {monday_record[key]}")
        ...
        for key, value in record.items():
            ...
            if key in date_columns:
                if isinstance(value, str):  # If it's a string
                    ...
                    try:
                        # Handle string date with time (like '1/23/2025 0:00')
                        if " " in value:
                            value = value.split(" ")[0]  # Remove time part
                            print(f"Date with time detected, using: {value}")  # Debug print
                        
                        # Try to parse the date in 'MM/DD/YYYY' format
                        date_obj = datetime.strptime(value, "%m/%d/%Y")
                        print(f"Parsed date (MM/DD/YYYY): {date_obj}")  # Debug print
...
# Example usage:
original_data = [
    {"Work Order": "WO123", "Date": "1/23/2025 0:00", "Amount": 1000, "Status": "Open"},
